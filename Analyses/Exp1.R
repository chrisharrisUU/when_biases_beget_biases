...
needs(BayesFactor, dplyr, ggplot2, ggsci, goallabr, gridExtra, here, lme4, lmerTest, magrittr, papaja, purrr, tidyr)
prioritize(dplyr)

# Data import
source("Auxiliary/Exp1_import.r")
...
# Graph over trials
time_series(df1)
# ggsave(filename = "Output/Graphs/Exp1_timeseries.svg", device = "svg")

# Choice model
m1 <- lmmtests(.data = df1,
         dv = "choice",
         between = "condition * (trial_reduced + I(trial_reduced^2))")
summary(m1)

m2 <- lmmtests(.data = df1, dv = "shift", between = "condition * bin_out")
m3 <- lmmtests(.data = df1, dv = "shift", between = "condition + bin_out + extremity")
m4 <- lmmtests(.data = df1, dv = "shift", between = "condition * bin_out * extremity")
anova(m2, m4)
summary(m3)
summary(m4)
...
# grid.arrange(exp1_pg1, exp1_pg2, ncol = 2) %>%
#   ggsave(plot = ., filename = "Output/Graphs/Exp1_postprefcond.svg", device = "svg", dpi = 320, width = 9.08, height = 5.72)
